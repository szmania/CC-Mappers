...
def run_high_confidence_unit_pass(root, tier, unit_variant_map, ck3_maa_definitions, unit_to_class_map, unit_to_description_map,
                                   ...
                                   faction_to_heritage_map=None, heritage_to_factions_map=None, faction_to_heritages_map=None, first_pass_threshold=0.90, llm_helper=None, faction_to_json_map=None, all_faction_elements=None):
    """
    ...
    for faction in factions_to_iterate:
        ...
        for element in faction.findall('MenAtArm'):
            ...
            # --- NEW: Exclude all units already used in this faction (professional units must be unique) ---
            ...
            all_used_keys = professional_keys | conscript_keys
            
            # If this element already has a key, remove it from exclusions to allow re-selection
            current_key = element.get('key')
            if current_key and current_key in all_used_keys:
                all_used_keys.discard(current_key)
                
            combined_exclusions.update(all_used_keys)
            print(f"    -> Excluding {len(all_used_keys)} units already used in faction from MenAtArm consideration.")
            # --- END NEW ---
    ...
    if failures:
        ...
        for failure_data in failures:
            ...
            if tag_name == 'LevyComposition':
                ...
                if cached_composition:
                    ...
                    changes, _ = unit_management.manage_faction_levies(
                        ...
                        excluded_units_set=combined_exclusions, destructive_on_failure=False, # Don't destroy if LLM provided a plan
                    ...
                    changes, _ = unit_management.manage_faction_levies(
                        ...
                        unit_to_class_map, faction_to_json_map.get(faction_name) if faction_to_json_map else {}, all_units, tier, # unit_to_tier_map is not used in manage_faction_levies
                        faction_elite_units=faction_elite_units, faction_name=faction_name,
                        excluded_units_set=combined_exclusions, destructive_on_failure=False,
            ...
            if tag_name == 'MenAtArm':
                ...
                if internal_type:
                    # Exclude all units already used in this faction (professional units must be unique)
                    professional_keys = faction_xml_utils.get_professional_keys_in_faction(faction_element)
                    conscript_keys = faction_xml_utils.get_conscript_keys_in_faction(faction_element)
                    all_used_keys = professional_keys | conscript_keys
                    ...
                    print(f"    -> (Low-Confidence) Excluding {len(all_used_keys)} units already used in faction for MAA '{maa_definition_name}'.")

                    # Get tiered pools specifically for this fallback logic
                    tiered_pools_for_fallback, tiered_log_strings = faction_xml_utils.get_all_tiered_pools(
                        faction_name, faction_pool_cache, screen_name_to_faction_key_map, faction_key_to_units_map,
                        faction_to_subculture_map, subculture_to_factions_map, faction_key_to_screen_name_map,
                        culture_to_faction_map, set(), faction_to_heritage_map,  # Use empty excluded_units_set
                        heritage_to_factions_map, faction_to_heritages_map
                    )
                    
                    accumulated_pool = set()

                    if tiered_pools_for_fallback:
                        for i, tier_pool in enumerate(tiered_pools_for_fallback):
                            ...
                            if combined_exclusions:
                                current_search_pool = current_search_pool - combined_exclusions
                            if all_used_keys:
                                current_search_pool = current_search_pool - all_used_keys
                    ...
                    if not found_key:
                        ...
                        if combined_exclusions:
                            global_pool = global_pool - combined_exclusions
                        if all_used_keys:
                            global_pool = global_pool - all_used_keys
...
def ensure_levy_structure_and_percentages(root, unit_categories, screen_name_to_faction_key_map, faction_key_to_units_map, template_faction_unit_pool, faction_to_subculture_map=None, subculture_to_factions_map=None, faction_key_to_screen_name_map=None, culture_to_faction_map=None, unit_to_class_map=None,
    ...
    for faction in factions_to_iterate:
        ...
        # --- NEW: Exclude professional units (Generals, Knights, MenAtArms) from levy selection ---
        professional_keys = faction_xml_utils.get_professional_keys_in_faction(faction)
        combined_excluded_units.update(professional_keys)
        print(f"  -> Excluding {len(professional_keys)} professional units from levy selection for faction {log_faction_str}.")
        # --- END NEW ---
...
def ensure_garrison_structure(root, unit_categories, screen_name_to_faction_key_map, faction_key_to_units_map, template_faction_unit_pool, faction_to_subculture_map=None, subculture_to_factions_map=None, faction_key_to_screen_name_map=None, culture_to_faction_map=None, unit_to_class_map=None, general_units=None, unit_to_training_level=None, tier=None, unit_to_tier_map=None, excluded_units_set=None, faction_pool_cache=None, faction_to_heritage_map=None, heritage_to_factions_map=None, faction_to_heritages_map=None, destructive_on_failure=True,
                                 faction_to_json_map=None, all_units=None, faction_culture_map=None, is_submod_mode=False, factions_in_main_mod=None, all_faction_elements=None): # Added faction_to_json_map and all_units
    """
    ...
    for faction in factions_to_iterate:
        ...
        # --- NEW: Exclude professional units (Generals, Knights, MenAtArms) from garrison selection ---
        professional_keys = faction_xml_utils.get_professional_keys_in_faction(faction)
        combined_excluded_units.update(professional_keys)
        print(f"  -> Excluding {len(professional_keys)} professional units from garrison selection for faction {log_faction_str}.")
        # --- END NEW ---
...
def run_final_fix_pass(root, validation_failures, categorized_units, all_units, unit_categories, tier, unit_variant_map, ck3_maa_definitions, unit_to_description_map, unit_stats_map, general_units, unit_to_class_map, excluded_units_set, screen_name_to_faction_key_map, faction_key_to_units_map, faction_to_subculture_map, subculture_to_factions_map, faction_key_to_screen_name_map, culture_to_faction_map, faction_to_heritage_map, heritage_to_factions_map, faction_to_heritages_map, faction_pool_cache, faction_to_json_map=None, faction_culture_map=None, llm_helper=None, unit_to_training_level=None, faction_elite_units=None, all_faction_elements=None):
    ...
    for failure_data in sorted_failures:
        ...
        if tag_name in ['General', 'Knights', 'MenAtArm']:  # Professional units
            # Must be unique across all unit slots
            all_used_keys = professional_keys | conscript_keys
            # If this element already has a key, remove it from exclusions to allow re-selection
            if element_to_fix is not None:
                current_key = element_to_fix.get('key')
                if current_key and current_key in all_used_keys:
                    all_used_keys.discard(current_key)
            combined_exclusions_for_faction.update(all_used_keys)
            print(f"  -> (Final Fix) Excluding {len(all_used_keys)} units already used in faction for professional unit '{tag_name}'.")
        elif tag_name in ['Levies', 'Garrison']:  # Conscript units
            # Cannot use professional units, but can share with other conscripts
            combined_exclusions_for_faction.update(professional_keys)
            print(f"  -> (Final Fix) Excluding {len(professional_keys)} professional units from conscript unit '{tag_name}' selection.")
        # --- END NEW ---
        ...
        if tag_name == 'LevyComposition':
            ...
            if cached_composition:
                ...
                changes, _ = unit_management.manage_faction_levies(
                    ...
                    faction_elite_units=faction_elite_units, faction_name=faction_name,
                    excluded_units_set=combined_exclusions_for_faction, destructive_on_failure=False,
                    levy_composition_override=cached_composition
                )
                if changes:
        ...
        replacement_successful = unit_management.find_and_apply_best_effort_replacement(
            ...
            unit_to_class_map, # Added unit_to_class_map parameter
            rank=rank,
            level=level,
